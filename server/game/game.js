...
Game.prototype = {
  ...
  addPlayer: function(player) {
    // Is the game full ?
    if (this.playerStack.length >= this.config.game.NB_PLAYER) return false;

    // Cannot join a started game
    if (this.state == this.config.gameStates.STARTED) return false;

    this.playerStack.push(player);

    if (this.playerStack.length >= this.config.game.NB_PLAYER) this.state = this.config.gameStates.READY;
  ...
  removePlayer: function(player) {
    ...
    if (this.state >= this.config.gameStates.READY) return false;
  ...
  addBot: function(bot) {
    if (this.botStack.length >= (this.config.game.MAX_NINJA - this.config.game.NB_PLAYER)) return false;
  ...
  processFrame: function() {
    ...
    if (this.state == this.config.gameStates.STARTED) this.sendMapUpdate();
  },
